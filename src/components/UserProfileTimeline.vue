  ...
  export default {
    ...
    methods: {
      ...
      vote(option) {
        ...
        this.axios.put(`https://localhost:4000/api/poll/${this.shownPoll.participant.id}`, voteObject).then(response => {
      ...
      shareTwitter(poll) {
       ...
       var finished = newString.join("");
        ...
        var text =
          `http://twitter.com/intent/tweet?text=${this.$store.state.userCred.userName}+just+shared+the+poll+'+${finished}+'+Click+the+link+below+to+cast+a+vote+https://localhost:8089/poll/shared/${poll.pollID}`
       console.log(text)
      }
    },
