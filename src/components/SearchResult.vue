  ...
  export default {
    ...
    methods: {
      ...
      createPoll() {
        ...
        this.axios.post('https://localhost:4000/api/poll/new', this.pollObject).then(response => {
      ...
      getUserData() {
        this.axios.get(`https://localhost:4000/api/user/${this.userID}`).then(response => {
          this.userCred = response.data
        })
      },
      fetchEventData() {
        this.axios.get(`https://localhost:4000/api/events/user/${this.userID}`).then(response => {
